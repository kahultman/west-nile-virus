...
The analysis will follow an iterative approach to finding a production model for West Nile forecasting. There are several data science iterative methods, and I will use the [CRISP](./images/CRISP.png) model for organizing this project. 
...
ggplot(train, aes(x=WnvPresent, y=NumMosquitos)) + geom_jitter(aes(color=WnvPresent)) + ggtitle("Number of mosquitos in West Nile \nVirus negative and positive traps") + xlab("West Nile Virus present in trap") + ylab("Number of mosquitos in trap")

train %>% group_by(Trap, Week) %>% 
  summarise(WNVrate = sum(WnvPresent)/n(), NumMosPer = sum(NumMosquitos)/n()) %>% 
  ggplot(aes(NumMosPer, WNVrate)) + geom_point() + stat_smooth() +
...
I next wanted to go back and examine the distribution of the Mosquito counts a bit further. 
...
## Data understanding: Number of mosquitos, again
...
train$WnvPresent <- as.character(train$WnvPresent)
train$WnvPresent[train$WnvPresent== "TRUE"] <- "WNV.Pos"
train$WnvPresent[train$WnvPresent== "FALSE"] <- "WNV.Neg"
train$WnvPresent <- as.factor(train$WnvPresent)

set.seed(300)
glmboost <- train(WnvPresent ~ nrows + precip10d + ma10 + Tmin + Summer + Year,
              data = train, 
              method = "glmboost",
              metric = "ROC",
              trControl = ctrl,
...
train$glmboos.Pred <- predict(glmboost, newdata = train, type = "prob")
test$glmboost.Pred <- predict(glmboost, newdata = test, type = "prob")
test$glmboos.Pred2 <- test$glmboost.Pred[,2]

set.seed(300)
glmboost2 <- train(WnvPresent ~ nrows + precip10d + ma10 + Tmin + Summer + Year + geo.hot.cl + WnvAvg,
              data = train, 
              method = "glmboost",
              metric = "ROC",
              trControl = ctrl,
...
glmboost2
glmboost2.Pred <- predict(glmboost2, newdata = train, type = "prob")
train$glmboos2.Pred2 <- glmboost2.Pred[,2]
glmboost2.Pred <- predict(glmboost2, newdata = test, type = "prob")
test$glmboos2.Pred2 <- glmboost2.Pred[,2]
summary(train$geo.hot.cl)
```
...
trainC50df <- train %>% select(WnvPresent, Species, nrows, Summer, WeekAvgMos, Tmin, WetBulb, ResultSpeed, ma10, precip3d, precip5d, precip10d)
