...
summary(lm2)
ggplot(data = train, aes(x=lm2NumMosq, y=(NumMosquitos - lm2NumMosq))) + geom_point(alpha = 0.05) + xlab("Predicted number of Mosquitos in training set") + ylab("Residual error (Actual - Predicted)") + ggtitle("Residual plot for estimating Mosquito number using linear model on weather data") + stat_smooth(method=lm)
...
train$glm1NumMosq <- predict(lm1, newdata = train)
test$glm1NumMosq <- predict(lm1, newdata = test)
...
Honestly, I'm not sure this is much better, but I'll stick with Poisson given the distribution. 
