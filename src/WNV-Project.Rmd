...
test$NumMoslmPred <- predict(lm1, newdata = test)
```
...
test$NumMosQuartile <- with(test, cut(NumMoslmPred, breaks=quantile(train$NumMosquitos, probs=seq(0,1, by=0.25), na.rm=TRUE), include.lowest=TRUE))
...
weeklyAvgNumMosq <- test %>% group_by(Week) %>% summarise(WeekAvgMos = mean(NumMoslmPred)) %>% ungroup()
test <- left_join(test, weeklyAvgNumMosq, by = "Week")
...
?predict
testpred_df$C50.Wnv.pred <- predict(c50m, newdata = test, type = "prob")
